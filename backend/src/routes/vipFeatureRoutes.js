...
const fetchUserRole = async (pool, userId) => {
  const result = await pool
    ...
    .query(
      "SELECT TOP 1 RoleID FROM Users WHERE FirebaseUserID = @UserID"
    );
  ...
  async (req, res) => {
    ...
    try {
      ...
      const totalApplicants =
        countResult.recordset[0]?.TotalApplicants || 0;
...
router.post("/employer/reveal-contact", checkAuth, async (req, res) => {
  ...
  try {
    ...
    if (!application) {
      return res.status(404).json({
        message:
          "Không tìm thấy hồ sơ ứng tuyển phù hợp với yêu cầu của bạn.",
      });
    }
